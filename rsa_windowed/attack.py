...
exp_file_suffix = 'exp'
pattern_files = 's'
pattern_file_0 = 'pattern_short_0.npy'
pattern_file_1 = 'pattern_short_1.npy'
...
def usage(argv):
    ...
    print('Options:')
    print('   [-sp | --short-pattern-files] <file_0.npy> <file_1.npy> : Specify short pattern files corresponding to the mask computation respectively for values 0 and 1 (default: \'pattern_0.npy\' and \'pattern_1.npy\')')
    print('   [-mp | --medium-pattern-files] <file_0.npy> <file_1.npy> : Specify medium pattern files corresponding to the mask computation respectively for values 0 and 1 (default: \'pattern_0.npy\' and \'pattern_1.npy\')')
    print('   [-lp | --short-pattern-file]  <file.npy>y>              : Specify a large pattern file corresponding to the entire ROI')
...
while idx < len(sys.argv):
    if sys.argv[idx] == '-sp' or sys.argv[idx] == '--short-pattern-files':
        pattern_files = 's'
        idx += 1
        if idx < len(sys.argv):
    ...
    elif sys.argv[idx] == '-mp' or sys.argv[idx] == '--medium-pattern-files':
        pattern_files = 'm'
        idx += 1
        if idx < len(sys.argv):
            pattern_file_0 = sys.argv[idx]
            ...
            if idx < len(sys.argv):
                pattern_file_1 = sys.argv[idx]
    elif sys.argv[idx] == '-lp' or sys.argv[idx] == '--large-pattern-file':
        pattern_files = 'l'
        idx += 1
        if idx < len(sys.argv):
            pattern_file_0 = sys.argv[idx]
...
if pattern_files == 's' or pattern_files == 'm':
...
cmd = ['gcc', '-O3', 'attack_rsa_windowed.c', '-o', 'attack_rsa_windowed']
...
for trace_file in trace_file_list:
    ...
    if pattern_files == 's':
        cmd = ['./attack_rsa_windowed', '-t', raw_trace_file, '-sp', raw_pattern_file_0, raw_pattern_file_1]
    elif pattern_files == 'm':
        cmd = ['./attack_rsa_windowed', '-t', raw_trace_file, '-mp', raw_pattern_file_0, raw_pattern_file_1]
    else:
        assert(pattern_files == 'l')
        cmd = ['./attack_rsa_windowed', '-t', raw_trace_file, '-lp', raw_pattern_file_0]
    print('# Running command: %s' % ' '.join(map(lambda x: '%s' % x, cmd)))
